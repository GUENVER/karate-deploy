...
function admin_site_form(?array $s): void
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $data = [
            ...
            'gen' => isset($f['gen']) ? 1 : 0, 'ymyl' => isset($f['ymyl']) ? 1 : 0, 'jobs' => isset($f['jobs']) ? 1 : 0, 'fuel' => isset($f['fuel']) ? 1 : 0, 'permalink' => trim($f['permalink'] ?? '/%slug%/') ?: '/%slug%/', 'lang' => 'fr',
        ];
    ...
    admin_page($s ? 'Modifier ' . $s['host'] : 'Ajouter un site', $msg . '<form method="post" class="box">' . csrf() . $hostField
        ...
        . '<p><label class="inline"><input type="checkbox" name="gen" ' . $ck('gen') . ' style="width:auto"> Génération IA automatique</label> &nbsp; <label class="inline"><input type="checkbox" name="adsense" ' . $ck('adsense') . ' style="width:auto"> AdSense</label> &nbsp; <label class="inline"><input type="checkbox" name="amazon" ' . $ck('amazon') . ' style="width:auto"> Affiliation Amazon</label> &nbsp; <label class="inline"><input type="checkbox" name="fuel" ' . $ck('fuel', 0) . ' style="width:auto"> Prix des carburants</label> &nbsp; <label class="inline"><input type="checkbox" name="jobs" ' . $ck('jobs', 0) . ' style="width:auto"> Agrégateur d\'offres d\'emploi</label> &nbsp; <label class="inline"><input type="checkbox" name="ymyl" ' . $ck('ymyl', 0) . ' style="width:auto"> Thème sensible (santé/argent : ton prudent, sources officielles)</label></p>'
