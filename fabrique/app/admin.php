...
function admin_site_form(?array $s): void
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $data = [
            ...
            'gen' => isset($f['gen']) ? 1 : 0, 'ymyl' => isset($f['ymyl']) ? 1 : 0, 'jobs' => isset($f['jobs']) ? 1 : 0, 'fuel' => isset($f['fuel']) ? 1 : 0, 'ev' => isset($f['ev']) ? 1 : 0, 'dpe' => isset($f['dpe']) ? 1 : 0, 'places' => isset(PLACE_MODULES[$f['places'] ?? '']) ? $f['places'] : '', 'permalink' => trim($f['permalink'] ?? '/%slug%/') ?: '/%slug%/', 'lang' => 'fr',
        ];
