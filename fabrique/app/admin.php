...
function admin_site_form(?array $s): void
{
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $data = [
            ...
            'gen' => isset($f['gen']) ? 1 : 0, 'ymyl' => isset($f['ymyl']) ? 1 : 0, 'jobs' => isset($f['jobs']) ? 1 : 0, 'fuel' => isset($f['fuel']) ? 1 : 0, 'ev' => isset($f['ev']) ? 1 : 0, 'dpe' => isset($f['dpe']) ? 1 : 0, 'commune' => isset($f['commune']) ? 1 : 0, 'places' => isset(PLACE_MODULES[$f['places'] ?? '']) ? $f['places'] : '', 'permalink' => trim($f['permalink'] ?? '/%slug%/') ?: '/%slug%/', 'lang' => 'fr',
        ];
    ...
    admin_page($s ? 'Modifier ' . $s['host'] : 'Ajouter un site', $msg . '<form method="post" class="box">' . csrf() . $hostField
        ...
        . '<p><label class="inline"><input type="checkbox" name="gen" ' . $ck('gen') . ' style="width:auto"> Génération IA automatique</label> &nbsp; <label class="inline"><input type="checkbox" name="adsense" ' . $ck('adsense') . ' style="width:auto"> AdSense</label> &nbsp; <label class="inline"><input type="checkbox" name="amazon" ' . $ck('amazon') . ' style="width:auto"> Affiliation Amazon</label> &nbsp; <label class="inline"><input type="checkbox" name="fuel" ' . $ck('fuel', 0) . ' style="width:auto"> Prix des carburants</label> &nbsp; <label class="inline"><input type="checkbox" name="ev" ' . $ck('ev', 0) . ' style="width:auto"> Bornes de recharge</label> &nbsp; <label class="inline"><input type="checkbox" name="dpe" ' . $ck('dpe', 0) . ' style="width:auto"> DPE par commune</label> &nbsp; <label class="inline"><input type="checkbox" name="commune" ' . $ck('commune', 0) . ' style="width:auto"> Fiches communes</label> &nbsp; <label class="inline"><input type="checkbox" name="jobs" ' . $ck('jobs', 0) . ' style="width:auto"> Agrégateur d\'offres d\'emploi</label> &nbsp; <label class="inline"><input type="checkbox" name="ymyl" ' . $ck('ymyl', 0) . ' style="width:auto"> Thème sensible (santé/argent : ton prudent, sources officielles)</label></p>'
        . '<p><label>Annuaire de lieux (données publiques)<select name="places"><option value="">— aucun —</option>' . implode('', array_map(fn($k, $mm) => '<option value="' . $k . '"' . ((($_POST['places'] ?? $s['places'] ?? '') === $k) ? ' selected' : '') . '>' . h($mm['title']) . '</option>', array_keys(PLACE_MODULES), PLACE_MODULES)) . '</select></label></p>'
